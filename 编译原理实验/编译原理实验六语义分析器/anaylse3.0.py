...
def LR(terminal,noterminal,ACTION,GOTO,inputList):
	#计数变量
	counter=1
	#状态栈
	state="0"
	...
	while 1:
		...
		print("%- 20s"%(state),end='\t')
		print("%- 20s"%(listToStr(mean)),end='\t')
		print("%- 20s"%(listToStr(symbal)),end='\t')
		print("%- 20s"%(listToStr(inputList)),end='\t')
		X=state[-1:]
		Y=symbal[-1]
		Z=inputList[0]
		if Z.isdigit():
